...
export class ProductVariationController {
    constructor(private productVariationUseCase: ProductVariationUseCase, private socketAdapter: SocketAdapter) {
    ...
    public async getCtrl(req: Request, res: Response) {
        try {
            ...
            if(!cmp_uuid || cmp_uuid.toLowerCase() === 'null' || cmp_uuid.toLowerCase() === 'undefined') {
                return res.status(400).json({
                    success: false,
                    message: 'No se pudo recuperar la variacion de producto.',
                    error: 'Debe proporcionar un Id de company.'
                });
            }
            ...
            const product = await this.productVariationUseCase.getDetailProductVariation(`${cmp_uuid}`,`${pro_uuid}`,`${prov_uuid}`)
            return res.status(200).send({
                success: true,
    ...
    public async insertCtrl({ body }: Request, res: Response) {
        try {
            ...
            const productByName = await this.productVariationUseCase.findProductVariationByName(cmp_uuid, pro_uuid, pro_name);
            if(productByName) {
                return res.status(400).json({
                    success: false,
                    message: 'No se pudo insertar el articulo.',
            ...
            const product = await this.productVariationUseCase.createProductVariation(body)
    ...
    public async updateCtrl(req: Request, res: Response) {
        try {
            ...
            const product = await this.productVariationUseCase.updateProductVariation(cmp_uuid, pro_uuid, prov_uuid, update)
            return res.status(200).json({
                success: true,
    ...
    public async deleteCtrl(req: Request, res: Response) {
        try {
            ...
            if(!cmp_uuid) {
                return res.status(400).json({
                    success: false,
                    message: 'No se pudo eliminar la variacion de producto.',
                    error: 'Debe proporcionar un Id de company.'
                });
            };
            if(!pro_uuid) {
            ...
            const product = await this.productVariationUseCase.deleteProductVariation(cmp_uuid, pro_uuid, prov_uuid)
            return res.status(200).json({
                success: true,
